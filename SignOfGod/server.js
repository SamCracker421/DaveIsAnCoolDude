...
app.get("/events/:name",(req,res)=>{
    db.run("select * from events where name=?",req.params.name,(err,row)=>{
    if(err){
        console.log(err);
    }
    if(row){
...
app.post('/signups', (req, res) => {
    ...
    db.serialize(function () {
        let init = db.prepare("insert into signups values (?,?,?,?,?)");
        init.run(signup.id, signup.userId, signup.eventId, signup.signout);
        init.finalize();
    })
...
db.each("select * from signups where userId=?",req.params.id,(err,row)=>{
    if(err){
        console.log(err)
    }
    if(row){
        row.signin=req.body.time;
        console.log(row);
        res.status(201).json(row);
    }
})
...
app.get("/students/:firstname/:lastname", (req, res) => {
    db.all("select * from students where firstname=? and lastname=?", [req.params.firstname, req.params.lastname], (err, row) => {
        if (err) {
...
app.get('/signups/:userId', (req, res) => {
    db.get("select * from signups where userId=?", req.params.userId, (err, row) => {
        if (err) {
            console.log(err)
            res.status(404)
        }
...
app.patch('/signups/:id', (req, res) => {
    db.get("select * from signups where id=?", req.params.id, (err, row) => {
        if (err) {
            ...
            db.run("update signups set userId=?,eventId=? ,signout=?,signin=?,signin=? where id=?",[userId,eventId,signout,signin,req.params.id],(e,r)=>{
                if(e){
                    console.log(e)
                }
                if(r){
...
app.delete('/students/:id', (req, res) => {
    db.all("delete from students where id=?", req.params.id, (row, err) => {
        if (err) {
            console.log(err)
        }
        if (row) {
...
app.delete('/signups/:id', (req, res) => {
    db.all("delete from signups where id=?", req.params.id, (row, err) => {
        if (err) {
            console.log(err)
        }
        if (row) {
